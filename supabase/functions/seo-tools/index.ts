...
const toolPrompts: Record<string, (body: any) => { system: string; user: string }> = {
  'keyword-generator': (body) => ({
    system: `أنت خبير SEO عربي متخصص في الأسواق العربية. تفهم اللهجات العربية المختلفة والمرادفات والأنماط اللغوية لكل دولة عربية. قدم النتائج بتنسيق JSON فقط.`,
    user: `ولّد 20-30 كلمة مفتاحية للموضوع: "${body.topic}"
الدولة العربية: ${body.countryNameAr || body.country}
العملة: ${body.countryCurrencySymbol || '$'}
${body.intent !== 'all' ? `نية البحث المطلوبة: ${body.intent}` : ''}

مهم جداً:
- راعِ اللهجة المحلية لهذه الدولة العربية (مثلاً: "موبايل" في مصر vs "جوال" في السعودية)
- أضف أسئلة بحثية شائعة بالعربية (ما هو، كيف، أفضل، أرخص)
- أضف مرادفات عربية وأشكال الجمع المختلفة
- أضف كلمات بالحروف اللاتينية إن كانت شائعة الاستخدام
- قدم بيانات CPC بالعملة المحلية
...
قدم كلمات متنوعة: رئيسية، طويلة الذيل، أسئلة، دلالية، لهجة محلية. أرقام البحث تقديرية منطقية.`
  }),
