...
const PreviewAnalysisSection: React.FC<PreviewAnalysisSectionProps> = ({ results, isAnalyzed = false }) => {
  // Only show this section when there are real results
  if (!isAnalyzed || !results || results.length === 0) return null;
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        >
          ...
          <h2 className="text-3xl md:text-4xl font-bold mb-4">
            نتائج تحليل <span className="gradient-text">موقعك</span>
          </h2>
          ...
          >
            ...
            <span>التقرير الكامل ({results.length} كلمة مفتاحية)</span>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="gradient-bg hover:bg-transparent border-0">
                    <TableHead className="text-primary-foreground font-bold text-right py-5 min-w-[150px]">الكلمة المفتاحية</TableHead>
                    <TableHead className="text-primary-foreground font-bold text-right py-5 min-w-[200px]">عنوان SEO</TableHead>
                    <TableHead className="text-primary-foreground font-bold text-right py-5 min-w-[250px]">وصف Meta</TableHead>
                    <TableHead className="text-primary-foreground font-bold text-center py-5">المنافسة</TableHead>
                    <TableHead className="text-primary-foreground font-bold text-center py-5">نية البحث</TableHead>
                    <TableHead className="text-primary-foreground font-bold text-center py-5">الاتجاه</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {results.map((item, index) => (
                    ...
                    >
                      ...
                      <TableCell className="text-right py-5 max-w-xs">
                        <span className="text-sm text-muted-foreground line-clamp-2">{item.metaDescription}</span>
                      </TableCell>
                      <TableCell className="text-center py-5">{getCompetitionBadge(item.competition)}</TableCell>
                      <TableCell className="text-center py-5">{getIntentBadge(item.searchIntent)}</TableCell>
